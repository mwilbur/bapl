...
local lineCount = 0
local updateLineCount = P(function() lineCount = lineCount + 1 return true end)
...
local base10_integer    = P"-"^-1*R"09"^1*I("num") / tonumber * ss
...
local grammar = P{ "test",
    ...
    statements  = collectAndApply(
                    statement*(SC*I("s1")*statements)^-1,
                    statementsAst),
    test       = identifier*assign*numeral*SC*identifier*assign*numeral
...
function run(code,store,io,stack,tracefunc) 
